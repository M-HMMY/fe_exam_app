...
| 区分 | 収録数 |
|---|---|
| 教本セクション | 68 節（本文 約 29.0 万字／目安 約 27 時間） |
| 図解 | 202 個（本文に埋め込み） |
| 一問一答 | 192 問（全 64 節に 3 問ずつ。採点も記録もしない自己確認用） |
| 対話ウィジェット | 15 種類 |
| 計算ドリル | 22 種類（毎回数値が変わる自動生成） |
| 科目A の問題 | 783 問（うち IPA 公開問題 127 問） |
| 科目B の問題 | 105 問／設問 168 問（うち IPA 公開問題 42 問） |
...
| 教本 | 入門編 4 節＋シラバス順の 64 セクション。各節は「ざっくり言うと」で始まり、一問一答と「この節のまとめ」で終わる。分野ごとに扉の導入文つき。全文検索・栞・理解度 3 段階の記録に対応し、節末から関連する節・計算ドリル・科目A/B の問題へ移動できる |
| 体験ツール | 15 個の対話ウィジェットを分野別に一覧・操作できる |
| 計算ドリル | 22 種類の計算問題を、毎回異なる数値で自動生成して繰り返し練習する |
| 科目A 演習 | 四肢択一。分野別／出題数指定／誤答優先。1 問ごとに解説と教本への導線 |
| 科目B 演習 | 擬似言語の読解問題と情報セキュリティの事例問題。行番号つきでプログラムを表示。全 105 問が教本の節にひも付き、解説から教本へ戻れる |
| 直前チェック | 教本 68 節の「まとめ」「試験のポイント」「よくある勘違い」計 550 項目を抜き出して一覧する。種類・分野・読了状況で絞り込める |
...
**オリジナルの練習問題**（大部分）… 出題形式と頻出テーマに沿って作成したもので、IPA の過去問そのものではありません。

**IPA 公開問題**（「公式過去問」バッジ付き）… 独立行政法人情報処理推進機構（IPA）が公開している実際の試験問題です。IPA は次のとおり明言しており、収録に許諾は不要です。

> 当機構で公表している過去の試験問題の使用に関し、法令に特別の定めがある場合を除き許諾や使用料は必要ありません。

収録元は [IPA 試験問題・解答例](https://www.ipa.go.jp/shiken/mondai-kaiotu/index.html) で公開されている以下の PDF です。

- 基本情報技術者試験 公開問題（令和5・6・7・8 年度）科目A・科目B
- 基本情報技術者試験 サンプル問題セット（2022 年公開）科目A・科目B

図が画像で描かれていて文字だけでは再現できない問題（アローダイアグラム、論理回路図、状態遷移図など）は収録していません。**解説は IPA が公開していないため、すべて本アプリで独自に書き起こしたものです。**

各問題の出典は問題カード下部に表示されます。実際の出題範囲は IPA のシラバスを確認してください。

---

## ★ 8 つめ（`kouatsugas_otsu_exam_app`）から配った道具（2026 年 9 月 20 日）

**このリポジトリには `CLAUDE.md` が無いので、ここに書いておきます。**
...
レビューのひな形は `scripts/prompts/` にあります。**計算ドリルと体験ウィジェットには、系譜 7 本ぶん誰もレビューを掛けていません。**

測った結果：**正解位置の偏りは問題なし**（24.0 / 24.4 / 25.2 / 26.4 %）。**解説が選択肢を位置の記号で指している問が 442 問中 104 問**あるので、**選択肢を並べ替えるなら解説も一緒に書き換えること。**
