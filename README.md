...
| 図解 | 193 個（本文に埋め込み） |
| 対話ウィジェット | 15 種類 |
| 計算ドリル | 22 種類（毎回数値が変わる自動生成） |
| 科目A の問題 | 656 問 |
| 科目B の問題 | 63 問（設問 126 問） |
...
| 計算ドリル | 22 種類の計算問題を、毎回異なる数値で自動生成して繰り返し練習する |
...
出題のたびに数値が変わる自動生成問題です（[src/data/drills.ts](src/data/drills.ts)）。計算問題は問題文ごと暗記してしまうと本番で崩れるため、手順だけが身に付くようにしています。

基数変換・2 の補数・シフト演算・CPI と MIPS・キャッシュの実効アクセス時間・稼働率・MTBF・待ち行列・伝送時間・サブネット（収容台数とネットワークアドレス）・ページフォールト回数・損益分岐点・減価償却・EVM・ファンクションポイント法・コミュニケーションパス・期待値・画像のデータ量・HDD のアクセス時間・2 分探索の比較回数・暗号鍵の必要数、の 22 種類。

誤答の選択肢には「ありがちな間違い」（バイトとビットの換算忘れ、ホスト数の −2 忘れ、回転待ちを 1 回転ぶんにする、直列と並列の式の取り違えなど）を優先して混ぜています。

種類を増やすには `DRILLS` 配列に `generate()` を持つオブジェクトを足すだけです。生成した問題は復習カード（SRS）には登録されません（同じ問題が二度と出ないため）。
