...
import { questionAById, questionBById } from '../data/questions';
...
type Item =
  | { kind: 'A'; qid: string; categoryId: string; question: ReturnType<typeof questionAById> }
  | { kind: 'B'; qid: string; categoryId: string; question: ReturnType<typeof questionBById>; subIndex: number };

function resolve(qid: string, categoryId: string): Item | null {
  if (qid.includes('#')) {
    const [id, idxText] = qid.split('#');
    const q = questionBById(id);
    const subIndex = Number(idxText);
    if (!q || Number.isNaN(subIndex) || !q.subQuestions[subIndex]) return null;
    return { kind: 'B', qid, categoryId, question: q, subIndex };
  }
  const q = questionAById(qid);
  if (!q) return null;
  return { kind: 'A', qid, categoryId, question: q };
}
...
export function Review(): JSX.Element {
  ...
  const applyGrade = useCallback(
    (g: Grade) => {
      ...
      if (!item) return;
      const answer = item.kind === 'A' ? item.question!.answer : item.question!.subQuestions[item.subIndex].answer;
    ...
    return (
      <div className="page">
        ...
        <div className="read-actions">
          <button type="button" className="btn primary" onClick={() => navigate('practice-a')}>
            科目A を演習する
          </button>
          <button type="button" className="btn" onClick={() => navigate('practice-b')}>
            科目B を演習する
          </button>
        </div>
      </div>
  ...
  const item = queue[idx];
  const correctAnswer =
    item.kind === 'A' ? item.question!.answer : item.question!.subQuestions[item.subIndex].answer;
  ...
  const counter = `${idx + 1} / ${queue.length}`;
